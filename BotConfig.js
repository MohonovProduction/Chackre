module.exports.Config = {
	whatsNew: 'меня отрефакторили во все модули и понизили чувствительность к гачи фразам изменив регулярку и накатив рандом (на голосовые тожк меньше реагирует) 😏',
	commands: [
		//{ command: '/', description: '' }
		{ command: '/whatsnew', description: 'что нового в последней версии' },
		{ command: '/love', description: 'любить' },
		{ command: '/fuck', description: 'послать' },
		{ command: '/add', description: 'добавить что-то' },
	]
}